...
class NonVersionedManyToManyRelationshipsTestCase(TestCase):
    def create_models(self):
        class Article(self.Model):
            __tablename__ = 'article'
            __versioned__ = {
    ...
    def test_relations_with_varying_transactions(self):
        ...
        assert len(article.versions[0].tags) == 1
        assert len(article.versions[1].tags) == 2
        assert len(article.versions[2].tags) == 0
        assert len(article.versions[3].tags) == 1


create_test_cases(NonVersionedManyToManyRelationshipsTestCase)
